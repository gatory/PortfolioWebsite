export default function PromotionSection() {
    return (
        <section className="flex flex-col items-center justify-center text-center px-6 py-16 bg-background text-foreground">
            ...
            <div className="flex flex-wrap gap-3 justify-center max-w-sm lg:max-w-lg mt-8 px-2">
                {["Full-Stack", "Data", "Embedded", "AI/Machine Learning"].map((focus) => (
                    <div 
                        key={focus} 
                        className="border border-zinc-800 bg-zinc-900/40 rounded-md px-4 py-2 font-bebas text-base lg:text-lg tracking-wider text-primary select-none hover:border-accent hover:text-accent transition-colors duration-300"
                    >
                        {focus.toUpperCase()}
                    </div>
                ))}
            ...
            <a 
                ...
                className="mt-10 bg-white hover:bg-amber-400 text-black font-bebas text-lg lg:text-xl tracking-widest px-8 py-3 rounded-lg font-bold hover:scale-[1.03] active:scale-[0.97] transition-all duration-300 shadow-[0_4px_25px_rgba(245,158,11,0.25)]"
            >
                Contact Him TODAY!
            </a>
        </section>
